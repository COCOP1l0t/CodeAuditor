...
- **Web settings boundary**: backend/log level, managed paths, and `max_concurrent_jobs` come only from `~/.code_auditor/settings.json`; browser requests cannot override them. Provider credentials (custom endpoint, API key, custom model) live in the SQLite `provider_settings` table, and `local` mode always uses the local CLI configuration for the model.
