...
# polished disclosure artifacts — similar complexity to Stage 6.
_MAX_TURNS = 500
...
def _vuln_id_from_report(report_path: str) -> str | None:
    """Extract vulnerability ID from a stage 6 report path.

    Expects paths like .../stage6-pocs/{vuln_id}/report.md
    """
...
def _find_finding_file(vuln_id: str, output_dir: str) -> str | None:
    """Locate the stage 5 evaluated finding JSON for a vulnerability ID."""
    path = os.path.join(output_dir, "stage5-vulnerabilities", f"{vuln_id}.json")
    return path if os.path.exists(path) else None


def _filter_reproduced(stage5_reports: list[str]) -> list[str]:
    """Keep only stage 6 reports from successful reproductions (no _fp suffix)."""
...
async def _run_disclosure(
    ...
    stage6_vuln_dir = os.path.join(config.output_dir, "stage7-disclosures", vuln_id)
...
async def run_stage7(
    ...
    if not reproduced:
        logger.info("Stage 7: No reproduced vulnerabilities to prepare disclosures for.")
        return []

    logger.info("Stage 7: Preparing disclosures for %d reproduced vulnerabilities.", len(reproduced))
    ...
    for i, (status, value, error) in enumerate(results):
        if i >= len(reproduced):
            continue
        if status == "rejected":
            logger.error("Stage 7: %s failed: %s", os.path.basename(reproduced[i]), error)
            continue
        if value:
            disclosure_reports.append(value)

    logger.info(
        "Stage 7 complete. %d disclosure packages prepared (from %d reproduced vulnerabilities).",
