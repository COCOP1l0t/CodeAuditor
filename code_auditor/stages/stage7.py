...
# Stage 7 agents verify reproduction, create minimal PoCs, and write
...
def _misuse_dir_exists(vuln_id: str, output_dir: str) -> bool:
    """Return True if Stage 6 flagged this vuln as API misuse."""
    return os.path.isdir(
        os.path.join(output_dir, "stage6-api-check", f"{vuln_id}_misuse")
    )


def _filter_eligible(stage5_reports: list[str], output_dir: str) -> list[str]:
    """Keep stage 5 reports from successful reproductions that were not flagged
    as API misuse by Stage 6."""
    eligible: list[str] = []
    for r in stage5_reports:
        if Path(r).parent.name.endswith("_fp"):
            continue
        vuln_id = Path(r).parent.name
        if _misuse_dir_exists(vuln_id, output_dir):
            logger.info("Stage 7: %s was flagged as api-misuse by Stage 6, skipping disclosure.", vuln_id)
            continue
        eligible.append(r)
    return eligible
...
async def _run_disclosure(
    ...
    if not vuln_id:
        logger.warning("Stage 7: Cannot extract vuln ID from %s, skipping.", report_path)
        return None

    key = _task_key(vuln_id)
    stage7_vuln_dir = os.path.join(config.output_dir, "stage7-disclosures", vuln_id)
    disclosure_dir = os.path.join(stage7_vuln_dir, "disclosure")
    disclosure_report = os.path.join(disclosure_dir, "report.md")

    if checkpoint.is_complete(key):
        logger.info("Stage 7: %s already complete, skipping.", vuln_id)
        return disclosure_report if os.path.exists(disclosure_report) else None

    logger.info("Stage 7: Starting disclosure preparation for %s.", vuln_id)
    ...
    log_file = os.path.join(stage7_vuln_dir, "agent.log")
    ...
    if not done:
        ...
        if not grace_done:
            logger.warning("Stage 7: %s agent task did not exit after cancel, moving on.", vuln_id)
        logger.warning(
            "Stage 7: %s timed out after %d minutes.",
    ...
    logger.info("Stage 7: %s complete (report=%s, timed_out=%s)", vuln_id, has_report, timed_out)
...
async def run_stage7(
    ...
    """Prepare disclosure artifacts for each non-misuse reproduced vulnerability in parallel."""
    eligible = _filter_eligible(cleared_reports, config.output_dir)
    if not eligible:
        logger.info("Stage 7: No eligible vulnerabilities to prepare disclosures for.")
        return []

    logger.info("Stage 7: Preparing disclosures for %d eligible vulnerabilities.", len(eligible))
    ...
    for i, (status, value, error) in enumerate(results):
        if i >= len(eligible):
            continue
        if status == "rejected":
            logger.error("Stage 7: %s failed: %s", os.path.basename(eligible[i]), error)
            continue
        if value:
            disclosure_reports.append(value)

    logger.info(
        "Stage 7 complete. %d disclosure packages prepared (from %d eligible vulnerabilities).",
        len(disclosure_reports), len(eligible),
    )
